 ...
 // Очистка состояния чекапа при переходе в другие разделы
  ...
  switch (text) {
    ...
    case "🔮 Повторить Чекап":
      ...
      storage.save(config.SETTINGS_FILE, userSettings);
      return await ctx.reply("✨ Напиши свою дату и время рождения (например: 26.08.1983 14:30), чтобы я построила твою карту.");

    case "❓ О методе":
      return await ctx.reply("Я — твой проводник в мире Сказкотерапии и Ба Цзы. Помогаю найти ответы внутри себя. Напиши Марии напрямую: @sherab_wangmo");
    ...
    case "✨ Курс-Путешествие":
      ...
      return await ctx.reply(
        ...
        `Если ты чувствуешь, что застрял в "Дне сурка" или играешь роль второго плана, пора переписать сюжет. Мой метод объединяет <b>коучинг, сказкотерапию и Ба Цзы</b>.\n\n` + 
    ...
    case "📈 Карта Пути (Сессия)":
      ...
      storage.save(config.SETTINGS_FILE, userSettings);
      return await ctx.reply(
        `📍 <b>Сессия «Картография души»</b> — это точка сборки твоего пути.\n\n` +
